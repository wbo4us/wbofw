...
// necessary classes
require_once('class.soapclient_w.php');
...
//		}
		...
		if (
			...
			$this->setError("Unsupported HTTP response status $http_status $http_reason (soapclient_w->response has contents of the response)");
			return false;
		}
	...
	*/
	...
	function getOperationData($operation, $bindingType = 'soap')
	{
		...
		foreach($this->ports as $port => $portData) {
            ...
            if (!empty($this->bindings[ $portData['binding'] ]['operations']))
...
/**
*
* soapclient_w higher level class for easy usage.
*
* usage:
*
* // instantiate client with server info
* $soapclient_w = new soapclient_w( string path [ ,boolean wsdl] );
*
* // call method, get results
* echo $soapclient_w->call( string methodname [ ,array parameters] );
*
* // bye bye client
* unset($soapclient_w);
*
...
* @access   public
*/
class soapclient_w extends nusoap_base  {
	...
	* @access   public
	*/
	function soapclient_w($endpoint,$wsdl = false,$proxyhost = false,$proxyport = false,$proxyusername = false, $proxypassword = false, $timeout = 0, $response_timeout = 30){
		parent::nusoap_base();
	...
	*/
	...
	";
		...
		$evalStr = 'class soap_proxy_'.$r.' extends soapclient_w {
	'.$evalStr.'
}';
		return $evalStr;
	}
    ...
    * @param array $cookie Cookie(name, value, expires, domain, secure)
